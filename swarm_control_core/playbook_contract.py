...
ALLOWED_COMMAND_IDS: Set[str] = {
    ...
    # explicit two-leg path primitive:
    # rotate(theta1) -> transit(length1) -> rotate(theta2-theta1) -> transit(length2)
    # used by playbook-1 "Move to objective" for non-synchronized independent
    # per-robot two-step objective arrivals.
    "transit_two_leg",
}
...
def validate_and_normalize(command_id: str, parameters_json: Optional[str]) -> Tuple[bool, str, Optional[ParsedPlaybook]]:
    ...
    north_m = 0.0
    east_m = 0.0
    if cid in ("transit_xy", "transit_two_leg"):
