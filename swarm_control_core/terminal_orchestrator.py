...
class TerminalOrchestrator(Node):
    ...
    def _collect_sequence_entry(
        ...
        Notes:
        - All playbooks capture variables up front and execute later.
        - Playbook 1 stores only objective and two-leg planner controls.
        """
    ...
    def _queue_entry_spec_line(self, entry: Dict[str, Any]) -> str:
        """
        Build a compact, human-readable one-line summary for a queued playbook.

        This line is shown directly under each numbered queue item in the
        EXECUTE PLAYBOOK SEQUENCE menu to make queued parameters obvious.
        """
        playbook = str(entry.get("playbook", ""))
        params = dict(entry.get("params", {}) or {})

        if playbook == "transit_distance":
            meters = float(params.get("meters", 0.0))
            direction = "forward" if meters >= 0.0 else "backward"
            return f"{direction} {abs(meters):g} m"

        if playbook == "rotate_degrees":
            degrees = float(params.get("degrees", 0.0))
            direction = "left" if degrees >= 0.0 else "right"
            return f"{abs(degrees):g} degrees {direction}"

        if playbook == "move_xy":
            x_m = float(params.get("x_m", 0.0))
            y_m = float(params.get("y_m", 0.0))
            return f"x={x_m:g} m, y={y_m:g} m, two-leg paths"

        if playbook == "execute_all_commands":
            speed = float(params.get("speed", 1.0))
            return f"primitive sweep, speed {speed:g}x"
    ...
    def _compute_move_xy_plans(
        self,
        detected_robots: List[str],
        x_m: float,
        y_m: float,
        max_detour_m: float,
    ) -> Tuple[Optional[Dict[str, TwoLegPlan]], Dict[str, float], Dict[str, str], str]:
        """
        Deterministically compute per-robot two-leg plans for playbook 1.

        Returns:
        - plans (or None on failure)
        - detour_targets
        - side_for_robot
        - error text
        """
        robots = [str(r) for r in detected_robots]
        if not robots:
            return None, {}, {}, "no detected robots"

        ref_line = (float(x_m), float(y_m))
        if math.hypot(ref_line[0], ref_line[1]) < 1e-9:
            ref_line = (1.0, 0.0)
        ...
        detour_targets: Dict[str, float] = {}
        n = max(1, len(robots))
        for i, robot in enumerate(robots, start=1):
            detour_targets[robot] = float(max_detour_m) * (float(i) / float(n + 1))

        plans: Dict[str, TwoLegPlan] = {}
        failures: List[str] = []
        for robot in robots:
            dx, dy = float(x_m), float(y_m)
            preferred_side = side_for_robot.get(robot, "left")
        ...
        return plans, detour_targets, side_for_robot, ""

    def _start_goal_async(
        self,
        robot: str,
        action_name: str,
        goal: ExecutePlaybook.Goal,
    ) -> Tuple[Optional[Dict[str, object]], Optional[GoalReport]]:
        """
        Start one goal for one robot without blocking for final result.

        Returns:
        - active state dict to poll later, or
        - immediate GoalReport on startup failure.
        """
        preview = self._build_motion_preview(robot, goal)
        self._emit_json_event(
            "task",
        ...
        if action_name.endswith("/execute_playbook_cmd"):
            ...
            try:
                ...
                reason = f"topic dispatch failed: {ex}"
                return None, GoalReport(robot=robot, accepted=False, success=False, reason=reason)
            self._emit_json_event(
                "audit",
                {
                    "type": "audit_event",
            ...
            return (
                {
                    ...
                    "deadline_s": time.monotonic() + 90.0,
                },
                None,
            )
        ...
        return (
            {
                ...
                "started_s": time.monotonic(),
                "deadline_s": time.monotonic() + max(0.2, float(self.send_goal_response_timeout_s)),
            },
            None,
        )

    def _poll_goal_async_state(self, state: Dict[str, object]) -> Optional[GoalReport]:
        """
        Poll one active async goal state.
        Returns GoalReport only when terminal (success/failure).
        """
        mode = str(state.get("mode", ""))
        robot = str(state.get("robot", ""))
        intent_id = str(state.get("intent_id", ""))
        now_mono = time.monotonic()

        if mode == "topic":
            key = (robot, intent_id)
            rep = self._topic_results.pop(key, None)
            if rep is not None:
                return rep
            if now_mono > float(state.get("deadline_s", now_mono)):
                return GoalReport(
                    robot=robot,
                    accepted=True,
                    success=False,
        ...
        if mode == "action_send":
            send_future = state.get("send_future")
            if send_future is None:
                return GoalReport(robot=robot, accepted=False, success=False, reason="missing send future")
            if not send_future.done():
                if now_mono > float(state.get("deadline_s", now_mono)):
                    return GoalReport(
                        robot=robot,
            ...
            if not goal_handle.accepted:
                return GoalReport(robot=robot, accepted=False, success=False, reason="goal rejected")
            self._active_goal_handles[robot] = goal_handle
            state["mode"] = "action_result"
            state["result_future"] = goal_handle.get_result_async()
            state["deadline_s"] = now_mono + 90.0
            return None

        if mode == "action_result":
            result_future = state.get("result_future")
            if result_future is None:
                self._active_goal_handles.pop(robot, None)
                return GoalReport(robot=robot, accepted=True, success=False, reason="missing result future")
            if not result_future.done():
                if now_mono > float(state.get("deadline_s", now_mono)):
                    self._active_goal_handles.pop(robot, None)
                    return GoalReport(robot=robot, accepted=True, success=False, reason="result timeout")
        ...
        return GoalReport(robot=robot, accepted=False, success=False, reason=f"unknown async mode '{mode}'")

    def _execute_sequence_pipeline(self, to_run: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Execute queue with per-robot pipelines (no fleet-wide step barriers).
        """
        robots_map = self._robots_map()
        target_pairs = self._target_actions(robots_map)
        target_robots = [r for r, _ in target_pairs]
        action_for_robot = {r: a for r, a in target_pairs}
        ...
        for idx, entry in enumerate(to_run, start=1):
            item = {
                "step": idx,
                "playbook": str(entry.get("playbook", "")),
                "label": str(entry.get("label", entry.get("playbook", "playbook"))),
                "status": "completed",
                "reason": "",
                "reports": [],
            }
            step_results.append(item)
            step_buckets[idx] = item

        # Per-robot command pipelines; each command points back to original queue step.
        per_robot_cmds: Dict[str, List[Dict[str, Any]]] = {r: [] for r in target_robots}
        ...
        for idx, entry in enumerate(to_run, start=1):
            ...
            if playbook == "move_xy":
                ...
                plans, _detours, _sides, err = self._compute_move_xy_plans(
                    detected_robots=detected_robots,
                ...
                if plans is None:
                    step_buckets[idx]["status"] = "failed"
                    step_buckets[idx]["reason"] = err or "planner failed"
                    continue

                speed = float(params.get("speed", self.default_speed_scale))
                for robot in detected_robots:
                    # Playbook 1 intentionally targets all detected robots.
                    # Keep this behavior even if target selector is narrowed.
                    if robot not in action_for_robot:
                        action_for_robot[robot] = robots_map.get(robot, f"/{robot}/execute_playbook_cmd")
                        per_robot_cmds.setdefault(robot, [])
                    p = plans.get(robot)
                    if p is None:
                        continue
                    g = ExecutePlaybook.Goal()
                    g.intent_id = f"term_seq_xy_{idx}_{robot}_{now}"
                    g.command_id = "transit_two_leg"
                    g.vehicle_ids = [robot]
                    g.parameters_json = json.dumps(
                        {
                            "theta1_deg": math.degrees(p.theta1),
                            "theta2_deg": math.degrees(p.theta2),
                            "length1_m": float(p.length1_m),
                            "length2_m": float(p.length2_m),
                            "speed": float(speed),
                        },
                        separators=(",", ":"),
                    )
                    per_robot_cmds.setdefault(robot, []).append(
                        {"step": idx, "playbook": playbook, "label": step_buckets[idx]["label"], "goal": g}
                    )
                continue

            targets = list(target_robots)
            if not targets:
                step_buckets[idx]["status"] = "skipped"
                step_buckets[idx]["reason"] = "no target robots"
                continue

            def _append_goal(robot: str, goal: ExecutePlaybook.Goal):
                per_robot_cmds.setdefault(robot, []).append(
                    {"step": idx, "playbook": playbook, "label": step_buckets[idx]["label"], "goal": goal}
                )
            ...
            if playbook == "execute_all_commands":
                ...
                for robot in targets:
                    for cidx, (command_id, cparams) in enumerate(command_plan, start=1):
                        g = ExecutePlaybook.Goal()
                        g.intent_id = f"term_seq_all_{idx}_{cidx}_{robot}_{now}"
                        g.command_id = command_id
                        g.vehicle_ids = [robot]
                        if command_id == "transit_xy":
                            g.north_m = float(cparams.get("north_m", 0.0))
                            g.east_m = float(cparams.get("east_m", 0.0))
                        g.parameters_json = json.dumps(cparams, separators=(",", ":"))
                        _append_goal(robot, g)
                continue
            ...
            if playbook == "transit_distance":
                ...
                for robot in targets:
                    ...
                    g = ExecutePlaybook.Goal()
                    g.intent_id = f"term_seq_transit_{idx}_{robot}_{now}"
            ...
            if playbook == "rotate_degrees":
                ...
                for robot in targets:
                    ...
                    g = ExecutePlaybook.Goal()
                    g.intent_id = f"term_seq_rotate_{idx}_{robot}_{now}"
                    g.command_id = "rotate"
                    g.vehicle_ids = [robot]
                    g.parameters_json = json.dumps(
                        {
            ...
            step_buckets[idx]["status"] = "skipped"
            step_buckets[idx]["reason"] = "unsupported sequence entry"

        # Mark steps with no mapped commands.
        for idx, item in step_buckets.items():
            has_cmd = any(cmd.get("step") == idx for cmds in per_robot_cmds.values() for cmd in cmds)
            if not has_cmd and item["status"] == "completed":
                item["status"] = "skipped"
                if not str(item.get("reason", "")).strip():
                    item["reason"] = "no target robots"

        # Runtime pipeline state
        robot_idx: Dict[str, int] = {r: 0 for r in per_robot_cmds.keys()}
        active: Dict[str, Dict[str, object]] = {}
        action_to_step: Dict[str, int] = {}

        while rclpy.ok():
            # Start next queued command for any robot that is idle.
            for robot, cmds in per_robot_cmds.items():
                if robot in active:
                    continue
                i = int(robot_idx.get(robot, 0))
                if i >= len(cmds):
                    continue
                goal = cmds[i]["goal"]
                action_name = action_for_robot.get(robot, f"/{robot}/execute_playbook_cmd")
                state, immediate = self._start_goal_async(robot, action_name, goal)
                if immediate is not None:
                    step_no = int(cmds[i]["step"])
                    step_buckets[step_no]["reports"].append(immediate)
                    robot_idx[robot] = i + 1
                    continue
                if state is not None:
                    active[robot] = state
                    action_to_step[robot] = int(cmds[i]["step"])

            # Stop when all robots drained and no active command remains.
            done_all = all(int(robot_idx.get(r, 0)) >= len(cmds) for r, cmds in per_robot_cmds.items())
            if done_all and not active:
                break

            # Handle emergency stop requests while pipelines are active.
            if self._poll_stop_keypress() or self._poll_stop_stdin():
                self._emergency_stop_all(show_ui=False)
                for robot in list(active.keys()):
                    step_no = int(action_to_step.get(robot, 0))
                    if step_no in step_buckets:
                        step_buckets[step_no]["reports"].append(
                            GoalReport(robot=robot, accepted=False, success=False, reason="stopped by user")
                        )
                break
            ...
            finished: List[str] = []
            for robot, state in active.items():
                rep = self._poll_goal_async_state(state)
                if rep is None:
                    continue
                step_no = int(action_to_step.get(robot, 0))
                if step_no in step_buckets:
                    step_buckets[step_no]["reports"].append(rep)
                robot_idx[robot] = int(robot_idx.get(robot, 0)) + 1
                finished.append(robot)
            for robot in finished:
                active.pop(robot, None)
                action_to_step.pop(robot, None)

        # Finalize step statuses from collected reports.
        for item in step_results:
            reports = list(item.get("reports", []))
            if not reports:
                continue
            if any(not (r.accepted and r.success) for r in reports):
                item["status"] = "failed"
                if not str(item.get("reason", "")).strip():
                    item["reason"] = "some robots failed"
            else:
        ...
        while rclpy.ok():
            ...
            if queue:
                ...
                for i, item in enumerate(queue, start=1):
                    ...
                    # Three-space indent requested by user for per-entry specs.
                    lines.append(f"   {self._queue_entry_spec_line(item)}")
            else:
                lines.append("")
            ...
            if choice in ("l", "load"):
                ...
                lines_show = [
                    "Select a queue JSON preset",
                    "",
                    f"Preset dir:",
                    self._default_presets_dir(),
                    "",
                    f"Preset queue files: {len(presets)}",
                ]
                if presets:
                    lines_show.extend([f"- {n}" for n in presets])
                else:
                    lines_show.append("- none found")
                lines_show.extend(
                    [
                        "",
                        "Enter preset name only",
                ...
                self._print_screen("LOAD QUEUE JSON", lines_show)
                path_raw = input(f"Preset name [{default_name}]: ").strip()
            ...
            if choice in ("e", "exec", "execute"):
                ...
                if not self._confirm(
                    [
                        ...
                        "Execute as per-robot pipelines?",
                    ]
                ):
                    continue
                # Snapshot queue for this run.
                to_run = list(queue)
                step_results = self._execute_sequence_pipeline(to_run)
    ...
    def _run_move_xy_with_params(
        ...
        # Playbook-1 target model:
        # - No main/anchor robot.
        # - Every robot receives the same objective vector in its local frame.
        # - Each robot is assigned a distinct two-leg path (side + detour level)
        #   so no robot takes the straight shortest path.
        goal_vectors: Dict[str, Tuple[float, float]] = {
            robot: (float(x_m), float(y_m)) for robot in detected_robots
        }
        ref_line = (float(x_m), float(y_m))
        if math.hypot(ref_line[0], ref_line[1]) < 1e-9:
            ref_line = (1.0, 0.0)

        # Alternate sides deterministically by robot order to make paths different.
        side_for_robot: Dict[str, str] = {}
        for i, robot in enumerate(detected_robots):
            side_for_robot[robot] = "left" if (i % 2 == 0) else "right"

        objective_heading = math.atan2(float(y_m), float(x_m)) if (abs(x_m) + abs(y_m)) > 1e-9 else 0.0

        # Assign non-zero detour targets for all robots so none use straight shortest path.
        detour_targets: Dict[str, float] = {}
        n = max(1, len(detected_robots))
        for i, robot in enumerate(detected_robots, start=1):
            detour_targets[robot] = max_detour_m * (float(i) / float(n + 1))
        ...
        summary = [
            ...
            "All robots use two-leg",
            "paths (no straight path)",
            "Side assignment: alternating",
            f"Objective heading={math.degrees(objective_heading):.1f} deg",
            f"MAX PATH OFFSET={max_detour_m:.2f} m",
            "",
            f"Est phase total: {est_total_seq:.2f} s",
            "",
            "Frame: robot-relative",
            "(shared objective vector)",
        ]
        ...
        for robot in detected_robots:
            ...
            summary.append(f"  mode={fallback_mode.get(robot, 'two_leg')}")
        ...
        self._emit_json_event(
            "task",
            {
                ...
                "parameters": {
                    ...
                    "y_m": y_m,
                    "speed": speed,
                    "side_assignment_mode": "alternating_by_robot_order",
                    "final_heading_mode": "two_leg_path",
                    "objective_heading_deg": math.degrees(objective_heading),
                ...
                "plans": {
                    robot: {
                        ...
                        "planner_mode": fallback_mode.get(robot, "two_leg"),
        ...
        # Independent dispatch: one two-leg command per robot.
        def build_goal(robot: str) -> ExecutePlaybook.Goal:
            p = plans[robot]
            g = ExecutePlaybook.Goal()
            g.intent_id = f"term_xy_independent_{robot}_{now}"
            g.command_id = "transit_two_leg"
            g.vehicle_ids = [robot]
            g.parameters_json = json.dumps(
                {
        ...
        phase_reports = self._dispatch_goal(build_goal, target_robots=detected_robots)
        if len(phase_reports) == 0:
            reason = (
                "Move-to-objective failed: zero dispatched targets. "
        ...
        ok = all(r.accepted and r.success for r in phase_reports)
        return {
            "ok": ok,
            "reason": "ok" if ok else "some robots failed",
