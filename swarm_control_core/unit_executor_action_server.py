    ...
    def __init__(self):
        ...
        if HAS_PLAYBOOK_ACTION:
            ...
            self.get_logger().warning(
                f"[{self.robot}] optional playbook action interface unavailable; "
                f"using standalone topic interface on {self.command_topic}"
            )
        ...
        *,
        ...
        try:
            if cid == "hold":
                ...
                rp = dict(parsed.raw_params or {})
                theta1_deg = float(rp.get("theta1_deg", 0.0))
                theta2_deg = float(rp.get("theta2_deg", theta1_deg))
                length1_m = float(rp.get("length1_m", 0.0))
                length2_m = float(rp.get("length2_m", 0.0))
                ...
                def _rot_twist(delta_rad: float) -> TimedTwistPlan:
                    t = Twist()
                    t.angular.z = w if delta_rad >= 0.0 else -w
                    dur = abs(delta_rad) / max(1e-6, abs(w))
                    txt = "rotate left" if delta_rad >= 0.0 else "rotate right"
                    return TimedTwistPlan(twist=t, duration_s=dur, status_text=txt)

                def _trans_twist(length_m: float) -> TimedTwistPlan:
                    t = Twist()
                    t.linear.x = v if length_m >= 0.0 else -v
                    dur = abs(length_m) / max(1e-6, abs(v))
                    txt = "transit forward" if length_m >= 0.0 else "transit backward"
                    return TimedTwistPlan(twist=t, duration_s=dur, status_text=txt)

                seq = [
                    _rot_twist(dtheta1),
                    _trans_twist(length1_m),
                    _rot_twist(dtheta2),
                    _trans_twist(length2_m),
                ]
                selected_strategy = "transit_two_leg"
                self._run_plan_sequence(seq, fb, composite_stop)
        finally:
