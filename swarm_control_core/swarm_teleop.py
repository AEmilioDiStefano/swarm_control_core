...
Arc turns (both tracks active; inner track reduced by configurable ratio):
- 7 forward-left
- 9 forward-right
- 1 backward-left
- 3 backward-right
...
class SwarmTeleop(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().error(f"[teleop] Required profile registry missing: {ex}")
            raise
        ...
        # Used for diff-drive arc-turn math (must match motor_driver_node).
        ...
        self._param_overrides = {
            ...
            "teleop_diff_arc_inner_ratio": float(self.get_parameter("teleop_diff_arc_inner_ratio").value) != float(teleop_diff_arc_inner_ratio),
        }
        ...
        # KEYMAP (7/9/1/3 handled specially for diff arc-turns)
        self.move_bindings = {
            # Arrow keys
            "\x1b[A": ("twist", 1, 0),     # up
            ...
            # Arc-turn keys (handled specially)
            "7": ("circle", None, None),   # forward-left
            "9": ("circle", None, None),   # forward-right
            "1": ("circle", None, None),   # backward-left
            "3": ("circle", None, None),   # backward-right
        }
    ...
    def render_instructions(self, topic_name: Optional[str]) -> str:
        ...
        lines.append(f"wheel_separation (for arc turns): {self.wheel_separation:.2f} m")
        ...
        if self.strafe_mode:
            ...
            lines.append("  7 arc forward-left")
            lines.append("  9 arc forward-right")
            lines.append("  1 arc backward-left")
            lines.append("  3 arc backward-right")
        lines.append("")
    ...
    def _apply_cmd_vel_topic(self, new_topic: str):
        ...
        self.publisher_ = self.create_publisher(Twist, new_topic, CMD_VEL_QOS)
    ...
    # ---------------- Track/Arc-turn helpers ----------------

    def _publish_track_speeds(self, v_left: float, v_right: float, description: str):
        """
        Publish an equivalent Twist for explicit left/right track speed targets:
        ...
        self._publish_and_log_twist(twist, description)

    def _publish_diff_arc_turn(self, forward_sign: float, turn_left: bool):
        """
        Differential-drive arc-turn helper for 7/9/1/3.

        Both tracks stay active; the inner track is scaled by
        `teleop_diff_arc_inner_ratio` to reduce scrub and current spikes.
        """
        outer = abs(self.linear_speed)
        inner_ratio = max(0.0, min(1.0, float(self.teleop_diff_arc_inner_ratio)))
        inner = outer * inner_ratio

        if turn_left:
            v_left = forward_sign * inner
            v_right = forward_sign * outer
            label = "arc-left"
        else:
            v_left = forward_sign * outer
            v_right = forward_sign * inner
            label = "arc-right"
        ...
        if forward_sign < 0:
            ...
            label += "-fwd"
        self._publish_track_speeds(v_left=v_left, v_right=v_right, description=label)
                ...
                if key in self.move_bindings:
                    ...
                    if mode == "circle":
                        ...
                        if drive_type in ("diff_drive", "diff", "diff-drive"):
                            if key == "7":
                                self._publish_diff_arc_turn(forward_sign=+1.0, turn_left=True)
                            elif key == "9":
