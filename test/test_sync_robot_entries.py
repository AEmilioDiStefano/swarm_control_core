...
def test_collect_sources_prints_registered_robots_one_per_line_before_prompt(capsys) -> None:
    with patch("swarm_control_core.sync_robot_entries._acquire_prompt_input", return_value=StringIO("\n")):
        sources = _collect_sources([], ready_robot_labels=["robot1@legion1.local", "robot2@legion2.local"])
    ...
    assert (
        ...
        "[SYNC]\n"
        "[SYNC]   robot1@legion1.local\n"
        "[SYNC]\n"
        "[SYNC]   robot2@legion2.local\n"
        "[SYNC]\n"
    ) in output
    assert output.index("[SYNC]   robot2@legion2.local") < output.index("Missing robot source")
...
""",
    ...
    assert _loadable_runtime_robot_labels([runtime_profiles]) == ["robot_ready@ready.local"]
