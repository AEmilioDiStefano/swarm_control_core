...
def test_step3_uses_defaults_only_when_ui_overrides_are_unset():
    ...
    assert 'export SWARM_CORE_THUMB_REFRESH_HZ="${SWARM_CORE_THUMB_REFRESH_HZ:-0.5}"' in text
    assert 'export SWARM_CORE_IMAGE_SUBSCRIPTION_MODE="${SWARM_CORE_IMAGE_SUBSCRIPTION_MODE:-active_only}"' in text
    assert 'export SWARM_CORE_IMAGE_THUMB_INTEREST_TTL_S="${SWARM_CORE_IMAGE_THUMB_INTEREST_TTL_S:-0.75}"' in text
    assert 'export SWARM_CORE_THUMB_ROBOTS_PER_TICK="${SWARM_CORE_THUMB_ROBOTS_PER_TICK:-0}"' in text
