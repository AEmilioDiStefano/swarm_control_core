...
if [[ -n "${SWARM_CORE_PROFILES_PATH:-}" ]]; then
  export PROFILES_PATH="$SWARM_CORE_PROFILES_PATH"
elif [[ -f "${runtime_cfg_dir}/robot_instances.yaml" ]]; then
  export PROFILES_PATH="${runtime_cfg_dir}/robot_instances.yaml"
fi
if [[ -n "${SWARM_CORE_CAMERA_PROFILES_PATH:-}" ]]; then
  export CAMERA_PROFILES_PATH="$SWARM_CORE_CAMERA_PROFILES_PATH"
elif [[ -f "${runtime_cfg_dir}/camera_profiles.yaml" ]]; then
...
# Apply the same multicast+unicast discovery policy used by the control UI,
...
echo "[swarm_core_run_robot] discovery_policy=${SWARM_CORE_DISCOVERY_MODE:-hybrid}"
echo "[swarm_core_run_robot] SWARM_DISCOVERY_MODE=${SWARM_DISCOVERY_MODE}"
echo "[swarm_core_run_robot] ROS_LOCALHOST_ONLY=${ROS_LOCALHOST_ONLY:-<unset>}"
echo "[swarm_core_run_robot] ROS_AUTOMATIC_DISCOVERY_RANGE=${ROS_AUTOMATIC_DISCOVERY_RANGE}"
echo "[swarm_core_run_robot] ROS_STATIC_PEERS=${ROS_STATIC_PEERS:-<none>}"
